...
function love.load()
	...
	--Dicionario (Tabela) que cria inimigos.
...
function love.update(dt)
	...
	--TRABALHO-06 - Enumeração
	--love.keyboard.isDown("right") é uma enumeração de "right" para checagem de tecla pressionada (right arrow key).
    ...
    for i, enemy in ipairs(enemies) do
		for j, bullet in ipairs(bullets) do
			if CheckCollision(enemy.x, enemy.y, enemy.image:getWidth(), enemy.image:getHeight(), bullet.x, bullet.y, bulletimg:getWidth(), bulletimg:getHeight()) then
				...
				--Acesso dinâmico de dados da tabela enemy para de fins de checagem de colisão ("enemy.x", "enemy.y")
				...
				--TRABALHO-06 - Arrays
				--Manipula a remoção de inimgios da tabela usando o "table.remove(enemies, i)"
